...
public sealed partial class BodyController : Component
{
	...
	[Property, Feature( "Input" )] public float WalkSpeed { get; set; } = 150;
	[Property, Feature( "Input" )] public float RunSpeed { get; set; } = 190;
	[Property, Feature( "Input" )] public float SprintSpeed { get; set; } = 320;
	[Property, Feature( "Input" )] public float JumpSpeed { get; set; } = 300;
	[Property, Feature( "Input" )] public float DuckedSpeed { get; set; } = 63.3f;
	...
	[Button( icon: "add" ), Property, Feature( "Animator" ), Tint( EditorTint.Green ), ShowIf( nameof( ShowCreateBodyRenderer ), true )]
	...
	protected override void OnFixedUpdate()
	{
		...
		IsNoclipping = Input.Pressed( "noclip" ) ? !IsNoclipping : IsNoclipping;
	...
	void InputMove()
	{
		var rot = new Angles( 0, EyeAngles.yaw, 0 ).ToRotation();
		...
		if ( Mode is Movement.MoveModeLadder ladderMode )
		{
		...
		{
			var velocity = RunSpeed;
			if ( Input.Down( "Walk" ) ) velocity = WalkSpeed;
			if ( Input.Down( "Run" ) ) velocity = SprintSpeed;
